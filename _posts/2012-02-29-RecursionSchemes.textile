...
|"хистоморфизм":http://cs.ioc.ee/~tarmo/papers/inf.ps.gz|"Histo":https://github.com/ekmett/recursion-schemes/blob/master/Data/Functor/Foldable.hs#L399|свёртка структуры на основе предыдущих вызовов (на каждом шаге доступ к результатам, полученным на предыдущих шагах), "аннотирует" каждый следующий элемент структуры результатом, полученным из предыдущих элементов||
...
|метаморфизм|"Gibbons":http://comonad.com/haskell/category-extras/src/Control/Morphism/Meta/Gibbons.hs|изменение репрезентации <span style="color:red;">(?)</span>||
